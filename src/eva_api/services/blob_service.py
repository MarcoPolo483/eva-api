...
class BlobStorageService:
    ...
    def __init__(self, settings: Settings):
        ...
        try:
            # Initialize Blob Storage client with timeout
            connection_string = (
    ...
    async def upload_document(
        ...
        if self.mock_mode or not self.container_client:
            logger.debug(f"Mock mode: Upload document {doc_id}")
            return {
                "id": str(doc_id),
                ...
                "size_bytes": 1024,
                "blob_url": f"https://mock-storage.blob.core.windows.net/documents/{blob_name}",
